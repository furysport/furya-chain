...
# check DENOM is set. If not, set to ufury
DENOM=${2:-ufury}
...
# check BINARY is set. If not, build furyad and set BINARY
if [ -z "$BINARY" ]; then
    make build
    BINARY=build/furyad
fi
...
$SED_BINARY -i 's/minimum-gas-prices = "0.25ufury"/minimum-gas-prices = "0.0ufury"/' $HOME_DIR/config/app.toml
