...
import (
	...
	config "github.com/furysport/furya-chain/v2/app/params"
...
func SetupApp(t *testing.T) *FuryaApp {
	t.Helper()
...
func SetupWithGenesisValSet(t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *FuryaApp {
	t.Helper()

	furyaApp, genesisState := setup(true)
...
func setup(withGenesis bool, opts ...wasmkeeper.Option) (*FuryaApp, GenesisState) {
	db := dbm.NewMemDB()
	furyaApp := NewFuryaApp(log.NewNopLogger(), db, nil, true, map[int64]bool{},
		DefaultNodeHome, 5, MakeEncodingConfig(), EmptyBaseAppOptions{}, opts)

	if withGenesis {
		return furyaApp, NewDefaultGenesisState()
	}

	return furyaApp, GenesisState{}
}

func genesisStateWithValSet(t *testing.T,
	app *FuryaApp, genesisState GenesisState,
