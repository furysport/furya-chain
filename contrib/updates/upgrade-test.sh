...
NODE1_HOME=node1/furyad
SELECTED_CONTAINER=furyadnode1
BINARY_OLD="docker exec $SELECTED_CONTAINER ./old/furyad"
TESTNET_NVAL=${1:-3}
...
docker exec $SELECTED_CONTAINER tar -cf ./furyad.tar -C . furyad
SUM=$(docker exec $SELECTED_CONTAINER sha256sum ./furyad.tar | cut -d ' ' -f1)
DOCKER_BASE_PATH=$(docker exec $SELECTED_CONTAINER pwd)
echo $SUM
UPGRADE_INFO=$(jq -n '
{
    "binaries": {
        "linux/amd64": "file://'$DOCKER_BASE_PATH'/furyad.tar?checksum=sha256:'"$SUM"'",
    }
}')
...
for (( i=0; i<$TESTNET_NVAL; i++ )); do
    ...
    if [[ $(docker ps -a | grep furyadnode$i | wc -l) -eq 1 ]]; then
        $BINARY_OLD tx gov vote 1 yes --from node$i --keyring-backend test --chain-id $CHAIN_ID --home "node$i/furyad" -y > /dev/null 2>&1
...
for (( i=0; i<$TESTNET_NVAL; i++ )); do
    if [[ $(docker ps -a | grep furyadnode$i | wc -l) -eq 1 ]]; then
        docker exec furyadnode$i ./furyad status --home "node$i/furyad"
        if [[ "${PIPESTATUS[0]}" != "0" ]]; then
            echo "node$i is not online"
            docker logs furyadnode$i
            exit 1
        fi
    else
        echo "furyadnode$i is not running"
        docker logs furyadnode$i
        exit 1
    fi
done