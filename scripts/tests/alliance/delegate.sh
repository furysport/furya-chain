...
IBC_TRANSFER=$($BINARY tx ibc-transfer transfer transfer channel-0 $VAL_WALLET_2 $AMOUNT_TO_DELEGATE$UFURY_DENOM --chain-id test-1 --from $VAL_WALLET_1 --home $CHAIN_DIR/test-1 --fees 60000$UFURY_DENOM --node tcp://localhost:16657 --keyring-backend test  -y -o json | jq -r '.raw_log' )
...
echo '{
  ...
  "metadata": "",
  "deposit": "25000000000'$UFURY_DENOM'",
...
PROPOSAL_HEIGHT=$($BINARY tx gov submit-proposal $CHAIN_DIR/create-alliance.json --from=$VAL_WALLET_2 --home $CHAIN_DIR/test-2 --node tcp://localhost:26657 -o json --keyring-backend test  --fees 60000$UFURY_DENOM  -y | jq -r '.height')
sleep 3
...
VOTE_RES=$($BINARY tx gov vote $PROPOSAL_ID yes --from=$VAL_WALLET_2 --home $CHAIN_DIR/test-2 --keyring-backend=test  --fees 60000$UFURY_DENOM --chain-id=test-2 --node tcp://localhost:26657 -o json -y)
...
DELEGATE_RES=$($BINARY tx alliance delegate $VAL_ADDR $AMOUNT_TO_DELEGATE$IBC_DENOM --from=node0 --from=$VAL_WALLET_2 --home $CHAIN_DIR/test-2 --keyring-backend=test  --fees 60000$UFURY_DENOM --chain-id=test-2 -o json -y)
sleep 3
...
TOTAL_SUPPLY_BEFORE_ALLIANCE=$($BINARY query bank total --denom $UFURY_DENOM --height $PROPOSAL_HEIGHT -o json | jq -r '.amount')
TOTAL_SUPPLY_AFTER_ALLIANCE=$($BINARY query bank total --denom $UFURY_DENOM -o json | jq -r '.amount')
...
if [ "$TOTAL_SUPPLY_INCREMENT" -gt 100000 ] && [ "$TOTAL_SUPPLY_INCREMENT" -lt 1000000 ]; then
    echo "Error: Something went wrong, total supply of $UFURY_DENOM has increased out of range 100_000 between 1_000_000. current value $TOTAL_SUPPLY_INCREMENT"
    exit 1
fi
