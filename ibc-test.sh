...
furyad tx intertx register --from demowallet2 --connection-id connection-0 --chain-id test-2 --home ./data/test-2 --keyring-backend test -y --broadcast-mode=block
...
export DEMOWALLET_2=$(furyad keys show -a demowallet2 --home ./data/test-2 --keyring-backend test)
furyad query intertx interchainaccounts connection-0 $DEMOWALLET_2
...
# icad tx ibc-transfer transfer transfer channel-1 furya1t7cyvydpp4lklprksnrjy2y3xzv3q2l075vlz3 1000000stake --chain-id=test-1 --from=demowallet1 --keyring-backend=test -y --broadcast-mode=block --home=./data/test-1 --node=http://localhost:16657

# furyad query bank balances furya1t7cyvydpp4lklprksnrjy2y3xzv3q2l075vlz3 
