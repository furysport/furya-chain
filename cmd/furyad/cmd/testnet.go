...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", sdk.DefaultBondDenom), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.001ufury)")
...
func NewTestnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator, validator genutiltypes.MessageValidator) *cobra.Command {
	...
	testnetCmd.AddCommand(testnetInitFilesCmd(mbm, genBalIterator, validator))
...
func initTestnetFiles(
	...
	furyaConfig := srvconfig.DefaultConfig()
	furyaConfig.MinGasPrices = args.minGasPrices
	furyaConfig.API.Enable = true
	furyaConfig.Telemetry.Enabled = true
	furyaConfig.Telemetry.PrometheusRetentionTime = 60
	furyaConfig.Telemetry.EnableHostnameLabel = false
	furyaConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", args.chainID}}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config", "app.toml"), furyaConfig)
	}
	...
	err := collectGenFiles(
		...
		args.outputDir, args.nodeDirPrefix, args.nodeDaemonHome, genBalIterator, validator,
	)
	if err != nil {
		return err
	}
